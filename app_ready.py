...
# CSS dla kolorów tła, spinnera i tekstu
st.markdown(
    """
    <style>
        .stApp { background-color: #F6F6EA; }
        /* Punkt 1 i 4: Kolor tekstu w spinnerze */
        .stSpinner > div > div {
...
if "base_selected_idx" not in st.session_state: st.session_state.base_selected_idx = 0
...
def fetch_street_suggestions(query: str) -> list[str]:
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("SELECT DISTINCT ulica, miejscowosc FROM dane.wypadki_lubuskie_przetworzone_2024 WHERE ulica ILIKE %s ORDER BY ulica LIMIT 10;", (f"%{query}%",))
        rows = cur.fetchall()
        conn.close()
        return [f"{u}, {m}" for u, m in rows if u]
    except: return []
...
def street_to_coords(street_city: str) -> Optional[Tuple[float, float]]:
    try:
        ...
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("SELECT AVG(gps_y_dec), AVG(gps_x_dec) FROM dane.wypadki_lubuskie_przetworzone_2024 WHERE ulica = %s AND miejscowosc = %s;", (street, city))
        row = cur.fetchone()
        conn.close()
        return (float(row[0]), float(row[1])) if row[0] else None
    except: return None
...
def fetch_accidents(route_coords):
    ...
    query = f'SELECT id, miejscowosc, ulica, gps_y_dec AS "lat", gps_x_dec AS "lon", unix_time FROM dane.wypadki_lubuskie WHERE gps_y_dec BETWEEN {min(lats)-0.01} AND {max(lats)+0.01} AND gps_x_dec BETWEEN {min(lons)-0.01} AND {max(lons)+0.01};'
    try:
        conn = get_db_connection()
...
# --- UI SIDEBAR ---
with st.sidebar:
    st.image("logo transparent.png", width='stretch')
    st.markdown("---")
    ...
    if st.button("Wyznacz trasę", width='stretch'):
...
if st.session_state.result:
    ...
    if not res["acc"].empty:
        ...
        for _, row in res["acc"].iterrows():
            folium.CircleMarker((row["lat"], row["lon"]), radius=5, color="red", fill=True, 
                                tooltip=f"Wypadek: {row['ulica']}, {row['miejscowosc']}").add_to(cluster)
    ...
    if not res["acc"].empty:
        ...
        final_dataframe['lon'] = [0 if str(x) == 'nan' else 1 for x in final_dataframe['lon']]
        final_dataframe.rename(columns={'lon': 'accidents'}, inplace=True)
        ...
        # Punkt 2: Zdanie o prawdopodobieństwie na czarno
